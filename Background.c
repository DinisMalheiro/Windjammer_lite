...
void BackgroundSetObject(Background *background, int index, Texture2D *texture, Vector2 position,
                         BackgroundLayer layer) {
  if (index < 0 || index >= MAX_BACKGROUND_OBJECTS || texture == 0)
    return;
...
void BackgroundDrawBehind(const Background *background) {
  // Animated background
  if (background->frameCount > 0 && background->frames[background->frame] != 0) {
  ...
  for (int i = 0; i < background->objectCount; i++) {
    ...
    if (object->texture != 0 && object->layer == BACKGROUND_LAYER_BEHIND) {
      DrawTexture(*object->texture, (int)object->position.x, (int)object->position.y, WHITE);
    }
  }
}
...
void BackgroundDrawFront(const Background *background) {
  ...
  for (int i = 0; i < background->objectCount; i++) {
    ...
    if (object->texture != 0 && object->layer == BACKGROUND_LAYER_FRONT) {
      DrawTexture(*object->texture, (int)object->position.x, (int)object->position.y, WHITE);
    }
  }
}
