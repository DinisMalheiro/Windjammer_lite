    ...
    switch (game->screen)
    {
        ...
        case GAME_SCREEN_PAUSE:
        {
            ...
            if (exitToMenu)
            {
                ...
                SoundManagerChangeMusic(
                    &game->sound,
                    "assets/Sounds/Start_song.wav"
                );
            }
...
static void GameDraw(Game *game)
{
    ...
    if (game->shakeTimer > 0.0f)
    {
        shakeX = (float)GetRandomValue(
            -(int)game->shakeStrength,
            (int)game->shakeStrength
        );

        shakeY = (float)GetRandomValue(
            -(int)game->shakeStrength,
            (int)game->shakeStrength
        );
    }
    ...
    Rectangle dest = {
        (GetScreenWidth() - width) / 2 + shakeX,
        (GetScreenHeight() - height) / 2 + shakeY,
        width,
        height
    };
