...
void CPUDraw(const CPU *cpu) {
  if (cpu->sprite != 0) {
    Rectangle sourceRec = {(float)(cpu->frame * cpu->frameWidth), (float)(cpu->currentRow * cpu->frameHeight),
                           -(float)cpu->frameWidth, (float)cpu->frameHeight};

    Rectangle destRec = {cpu->position.x, cpu->position.y, (float)cpu->frameWidth, (float)cpu->frameHeight};

    Vector2 origin = {cpu->frameWidth / 2.0f, cpu->frameHeight / 2.0f};
